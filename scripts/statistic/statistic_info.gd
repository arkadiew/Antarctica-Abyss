...
func get_task_summary() -> String:
	...
	var total := TaskManager.tasks.size()
	var completed := 0
	...
	var overall_percentage = (completed / total * 100.0) if total > 0 else 0.0
	result += "\nИтого выполнено: %d из %d (%.1f%%)" % [completed, total, overall_percentage]

	return result
	
	info_label.text = get_task_summary()
